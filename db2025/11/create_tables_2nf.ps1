# SQLite 実行ファイル
$sqlite = "./sqlite3.exe"   # PATH が通っていればこのままでOK
# 作成するDBファイル
$dbPath = "game_2nf.db"
...
-- ===== 2NF スキーマ =====
-- ※ 主キーはすべて単一。job_master は置かず、職業名は character に直接持たせる。
--    → 全テーブルは 2NF を満たすが、職業名の冗長性の分だけ 3NF からは外れる。
...
CREATE TABLE character (
  ...
  person_id      INTEGER,
  job_name       TEXT,    -- ★職業名をそのまま保持（マスタ分離しない）
  character_name TEXT,    -- 異世界名のみ
  HP             INTEGER,
  MP             INTEGER,
  EXP            INTEGER,
...
-- 職業名の候補（マスタ分離せず、一時テーブルからランダムに選んで character に埋め込む）
CREATE TEMP TABLE job_names_tmp (name TEXT);
INSERT INTO job_names_tmp (name) VALUES
('戦士'),
('魔法使い'),
('僧侶'),
('盗賊'),
...
-- character：job_name を直接保持（2NFは満たすが、3NF的には冗長）
INSERT INTO character (character_id, person_id, job_name, character_name, HP, MP, EXP)
SELECT
  p.person_id,
  p.person_id,
  (SELECT name FROM job_names_tmp ORDER BY RANDOM() LIMIT 1),
...
$sql | & $sqlite $dbPath
Write-Host "done: $dbPath に2NFスキーマとダミーデータを作成しました。"
